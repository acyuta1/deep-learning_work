...
def result(movie_copy,most_similar):
    ...
    watched_movies = pd.DataFrame(watched_list, columns=['movieId','Title','Genres','Tags'])
        
    for i in most_similar:
        if i not in movie_copy:
            recommendations_list.append(df.loc[df['movieId'] == i].values[0])
    rec_movies = pd.DataFrame(recommendations_list, columns=['movieId','Title','Genres','Tags'])
