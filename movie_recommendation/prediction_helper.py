...
def prepare_inputs(movie_input):
    ...
    for i in genres:
        for j in i.split():
            if '-' in j:
                genre_input.append(list(genres_map.keys())[list(
                    genres_map.values()).index(j.split('-')[0])])
                genre_input.append(list(genres_map.keys())[list(
                    genres_map.values()).index(j.split('-')[1])])
            else:
    ...
    for i in tags:
        for j in i.split():
            if '-' in j:
                tag_input.append(list(tag_map.keys())[list(
                    tag_map.values()).index(j.split('-')[0])])
                tag_input.append(list(tag_map.keys())[list(
                    tag_map.values()).index(j.split('-')[1])])
            elif '!' in j:
                tag_input.append(list(tag_map.keys())[list(
                    tag_map.values()).index(j.split('!')[0])])
            else:
